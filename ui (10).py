  ...
  try{
    ...
    if(mg.available){
      ...
      el.innerHTML=`<div class="small" style="margin:5px 0 8px">${mg.message||''}</div>`+
        items.map((p,idx)=>`<details class="puzzleChoiceDetail"><summary>${p.image_url?`<img class="puzzleChoiceThumb" src="${p.image_url}" alt="${p.name||'Puzzle'}" loading="lazy" onerror="this.style.display='none'">`:''}<span><strong>${idx+1}. ${p.name||'Puzzle'}</strong>${p.manufacturer?`<small>${p.manufacturer}</small>`:''}</span></summary><div class="puzzleChoiceBody">Letzte Zeit <strong>${p.last_time||'–'}</strong> · MSP-Median <strong>${p.median||'–'}</strong> · Abstand <strong>${p.gap||'–'}</strong></div></details>`).join('');
    }else{
...
async function loadUnsolvedLibrary(){
 ...
 return `<details class="puzzleChoiceDetail"><summary>${p.image_url?`<img class="puzzleChoiceThumb" src="${p.image_url}" alt="${p.name||'Puzzle'}" loading="lazy" onerror="this.style.display='none'">`:''}<span><strong>${p.name||'Puzzle'}</strong>${p.manufacturer?`<small>${p.manufacturer}</small>`:''}</span></summary><div class="puzzleChoiceBody">${p.pieces?p.pieces+' Teile':'Teilezahl unbekannt'}${diff}${pred}<br><span class="pill">noch kein Solo-Ergebnis</span></div></details>`}).join('');
...
async function loadRepeatPriority(){
 ...
 return `<details class="puzzleChoiceDetail"><summary>${p.image_url?`<img class="puzzleChoiceThumb" src="${p.image_url}" alt="${p.name||'Puzzle'}" loading="lazy" onerror="this.style.display='none'">`:''}<span><strong>${i+1}. ${p.name||'Puzzle'}</strong>${p.manufacturer?`<small>${p.manufacturer}</small>`:''}</span></summary><div class="puzzleChoiceBody">Priorität <strong>${p.score}/100</strong> · ${p.label}<br>Letzte ${p.latest}${prev} · Best ${p.best}${med}${days}<br>${(p.reasons||[]).join(' · ')}</div></details>`}).join('');
