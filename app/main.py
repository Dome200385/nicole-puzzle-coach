...
def _latest_snapshot(db):
    return db.query(SyncSnapshot).filter(SyncSnapshot.owner_key=="nicole").order_by(SyncSnapshot.id.desc()).first()

def _snapshot_payload(s):
    return {"profile":json.loads(s.profile_json),"results":json.loads(s.results_json),"statistics":json.loads(s.statistics_json),"collections":json.loads(s.collections_json)}
...
def manual_summary(db:Session=Depends(get_db)):
    rows=db.query(TrainingSession).order_by(TrainingSession.date.asc(),TrainingSession.id.asc()).all()
...
def countdown(db:Session=Depends(get_db)):
    ...
    return tournament_countdown(_tournament_dicts(rows))
...
def latest_data(db:Session=Depends(get_db)):
    ...
    if not s: raise HTTPException(404,"Noch keine Daten synchronisiert")
...
@app.get("/tournaments")
def list_tournaments(db:Session=Depends(get_db)): return _tournament_dicts(db.query(Tournament).order_by(Tournament.date.asc()).all())
...
def list_training_sessions(db:Session=Depends(get_db)): return _training_dicts(db.query(TrainingSession).order_by(TrainingSession.id.desc()).all())

@app.get("/coach/performance")
def coach_performance(mode:str="solo",pieces:int|None=None,db:Session=Depends(get_db)):
    s=_latest_snapshot(db)
    if not s: raise HTTPException(404,"Noch keine MySpeedPuzzling-Daten synchronisiert")
    p=_snapshot_payload(s); return performance_summary(p["results"],mode=mode,pieces=pieces)

@app.get("/coach/library")
def coach_library(db:Session=Depends(get_db)):
    s=_latest_snapshot(db)
    if not s: raise HTTPException(404,"Noch keine MySpeedPuzzling-Daten synchronisiert")
    p=_snapshot_payload(s); return owned_vs_history(p["results"],p["collections"])
...
def coach_readiness(tournament_id:int,db:Session=Depends(get_db)):
    ...
    return {"tournament":{"id":t.id,"name":t.name,"date":t.date},"readiness":tournament_readiness(sessions,{"mode":t.mode,"manufacturer":t.manufacturer,"piece_count":t.piece_count})}

@app.get("/coach/next-puzzle")
def coach_next_puzzle(tournament_id:int|None=None,db:Session=Depends(get_db)):
    s=_latest_snapshot(db)
    if not s: raise HTTPException(404,"Noch keine MySpeedPuzzling-Daten synchronisiert")
    p=_snapshot_payload(s); td=None
