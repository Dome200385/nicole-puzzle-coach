...
@app.get("/api")
def api_root():
    return {"app":"Nicole Puzzle Coach API","version":"6.11.3","status":"online","dashboard":"/dashboard","docs":"/docs"}
...
def pwa_service_worker():
    return Response(content="""const CACHE_NAME='nicole-puzzle-coach-v6113';
...
@app.get("/health")
def health(): return {"status":"ok","version":"6.11.3"}
...
async def msp_api_test(db:Session=Depends(get_db)):
    ...
    try:
        ...
        return {"ok":True,"mode":"pat","api_only":True,"user_agent":"NicolePuzzleCoach/6.11.3","player_id":profile.get("id") if isinstance(profile,dict) else None,"player_name":profile.get("name") if isinstance(profile,dict) else None}
    except Exception as exc:
        return {"ok":False,"mode":"pat","api_only":True,"user_agent":"NicolePuzzleCoach/6.11.3","error":str(exc)}
